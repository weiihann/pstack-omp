...
3. Pick the runners. Use the `arena runners` panel from `$PSTACK_CONFIG (or .pstack/config.md)` when present. Otherwise use a diverse host-supported panel drawn from `designer`, `planner`, `reviewer`, and `inherit-parent`; the live host inventory wins. An `auto` or `inherit-parent` entry in this line or the cross-judge line means the parent model, so omit `model` for it. If the host task runner rejects a configured entry, run that seat on its closest host-valid equivalent of the same family and say so. Spawn more when the arena covers multiple design directions. Repeating one host-supported choice N times is valid when the work is generation-bound rather than judgment-sensitive.
