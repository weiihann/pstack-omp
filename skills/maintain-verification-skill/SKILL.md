...
0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map (usually `skills/verify-*/` within this package, or the current host's project-local skill root). Several candidates → ask which one using the host interaction contract; none → stop and point at the sibling **create-verification-skill** skill instead of inventing a target.
...
2. **Source wave.** One canonical `explorer` child per feature file, read-only and launched concurrently when the host supports it. If child execution is unavailable, process the same slices locally and keep the return shape unchanged. Each explains "how does this user-facing feature work?" from source, flags likely doc drift with citations, and returns one concise live-verification recipe. Children never drive the app and never edit files. Return shape: feature summary / source entry points / likely drift or none / one recipe.
