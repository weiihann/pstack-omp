...
Look recursively in the current project's host-supported skill roots for `**/*-mode/SKILL.md` matching the user's handle. Within this package, use `skills/<handle>-mode/SKILL.md` or an established `skills/<handle>/<handle>-mode/SKILL.md` category. If one exists, confirm intent with the host's structured question facility, or one focused chat question when unavailable (unless they already said "update my skill" or similar):
...
Locate the active workspace's transcript source before fanning out. Use the host history resource, an explicit transcript directory, or `$PSTACK_TRANSCRIPTS_DIR`, in that order. Use only that workspace-scoped source. If none is available, report the gap and continue with the direct interview; never scan a global vendor history tree or another workspace.
...
Mining misses intent that hasn't come up yet. Use the host's structured multi-choice interaction when available; otherwise ask one focused question in normal chat.
...
Apply the sibling **unslop** skill and the live host's skill-authoring guidelines to every line.
